...
from utils import read_accounts_from_file, write_accounts_to_file, reset_balances, setup_logger, load_settings, get_max_games
        ...
        while retry_count < 3 and not success and not exit_flag:
            try:
                ...
                try:
                    ...
                    username = "N/A"
                    logger.warning(f"Failed to get username for account {account}: {e}")
                ...
                try:
                    ...
                    next_schedule = None
                    logger.warning(f"Failed to calculate next schedule for account {account}: {e}")

                try:
                    balance = parse_balance(bot.get_balance())
                except Exception as e:
                    balance = 0.0
                    logger.warning(f"Failed to parse balance for account {account}: {e}")
                # Обновление баланса
    ...
    with balance_lock:
        for account, timer_info in list(timers_data.items()):
            ...
            if next_schedule <= current_time:
                logger.info(f"Account {account}: Timer expired and removed from timers.")
